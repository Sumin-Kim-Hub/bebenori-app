...
LOGO_FILE    = "logo_jpg.png"          # ← 업로드된 로고
...
.qtag {{
    ...
    box-shadow: 0 2px 6px rgba(242,183,5,.1);
}}
...
# 퀵 필터 해시태그 → (표시 레이블, 검색 쿼리)
QUICK_TAGS = [
    ("#영유아 전용",        "영유아 전용 키즈카페 추천"),
    ("#에너지 발산",        "뛰어놀 수 있는 키즈카페"),
    ("#오감 놀이",          "오감 자극 감각 놀이 키즈카페"),
    ("#주말 나들이",        "주말에 아이랑 갈 키즈카페"),
    ("#수유실 있는 곳",     "수유실 있는 영아 친화 카페"),
    ("#예약 없이 바로",     "예약 없이 바로 갈 수 있는 곳"),
    ("#부모도 쉴 수 있는", "부모도 편하게 쉴 수 있는 키즈카페"),
]
...
# ─────────────────────────────────────────────────────────────
# 16. 퀵 필터 해시태그 (작은 추천 검색어 스타일)
# ─────────────────────────────────────────────────────────────
tag_items = " ".join(
    f'<span class="qtag">{label}</span>' for label, _ in QUICK_TAGS
)
st.markdown(
    f'<div class="quick-row">'
    f'<span class="quick-label">추천 검색어</span>{tag_items}</div>',
    unsafe_allow_html=True,
)
# 클릭 처리용 숨김 버튼 (st.button은 JS onclick이 없으므로 columns로 처리)
q_cols = st.columns(len(QUICK_TAGS))
for i, (label, query) in enumerate(QUICK_TAGS):
    with q_cols[i]:
        # 버튼은 CSS로 숨기고 해시태그 span 클릭 시 preset_q 주입
        if st.button(label, key=f"qt_{i}"):
            st.session_state.preset_q = query
            st.rerun()
